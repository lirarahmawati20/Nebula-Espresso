...
export async function login(req, res) {
    ...
    try {
        const user = await pool.query(`SELECT * FROM users WHERE role = 'admin' and  username = '${username}'`);
        ...
        const accessTokenExpiresIn = formatExpirationDate(new Date(Date.now() + process.env.ACCESS_TOKEN_EXPIRATION * 1000));
        const refreshTokenExpiresIn = formatExpirationDate(new Date(Date.now() + process.env.REFRESH_TOKEN_EXPIRATION * 1000));
...
export async function loginKasir(req, res) {
    ...
    try {
        const user = await pool.query(`SELECT * FROM users WHERE role = 'kasir' and  username = '${username}'`);
        if (user.rows.length === 0) {
            return res.status(404).send("User not found");
        }
        const passwordMatch = await bcrypt.compare(password, user.rows[0].password);
